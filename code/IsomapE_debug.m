function [Y, R, E, NG, RV_allDim, n_comps, k_values, trajNgbr, entropyReached] = IsomapE_debug(D, Meta, n_fcn, n_size, options); 

% ISOMAPE   Computes Isomap embedding using adaptive, entropy-based neighborhoods
%
% [Y, R, E, NG, RV_allDim, n_comps, k_values, trajNgbr, entropyReached] = \
%                                   isomapE(D, Meta, n_fcn, n_size, options); 
%
% Input:
%
%
...
%    
%
%    Meta = meta data describing each data point N x 1 (O R N x n_p, where n_p = num parameters)
%
%
...
%                  
%       options.entropythesh = h (float) desired entropy for neighborhoods.
%
%       options.max_incr_k = (int) maximum allowed increments in setting k_i
%                            to reach entropy threshold. (default = 100)
%
%
% Output: 
...
%    
%    NG = pairwise distances (if desired for further analysis)
%    RV_allDim = Residual variance for each embedding for d = 1 to dims
%    n_comps = number of connected components in graph, to verify = 1
%    k_values = the neighborhood size reached for each point (N x 1)
%    trajNgbr = the frequency of each label appearing in each neighborhood
%    entropyReached = final entropy value of each neighborhood (N x 1)

%    COPYRIGHT NOTICE FOR ENTROPY-ISOMAP EXTENSION
%    FOR COPYRIGHT INFORMATION OF ORIGINAL ISOMAP AUTHORS SEE BELOW
%
%    IsomapE code -- (c) 2018 SCoRe Group http://www.score-group.org/
...
%% DETERMINING THE NEIGHBPURHOOD FUNCTION
if n_fcn=='k'
     K = n_size; 
     if ~(K==round(K))
...
%% DEFINING INFINITY BASED ON MODE
if (mode == 3)
     INF = inf; 
else
...
if ~isfield(options, 'entropythresh')
     options.entropythresh = -1;
end
if ~isfield(options, 'paramfix')
     options.paramfix = 1;
end
if ~isfield(options, 'max_incr_k')
    options.max_incr_k = 100;
end


max_incr_k = options.max_incr_k;
paramfix = options.paramfix;
entropythresh = options.entropythresh;
dims = options.dims; 
comp = options.comp; 
...
elseif ((mode == 1) & (use_dijk == 1))
     if n_fcn == 'k' 
         [tmp, ind] = sort(D); 
         tic;
         k_values = zeros(size(D, 1), 1);
         trajValues = unique(Meta(1:end));
         ngbr_counts = zeros(size(trajValues));
         trajNgbr = containers.Map('KeyType','double','ValueType','any');

         for i=1:size(trajValues,2)
             trajNgbr(trajValues(i)) = zeros(size(trajValues));
         end

         entropyReached = zeros(N, 1);
         for i=1:N
             if entropythresh ~= -1
                 [Hr, Tc, eV] = findEntropy(Meta, D(i, 1:end),  ind(1:end, i), K, entropythresh, max_incr_k);
                 entropyReached(i) = eV;
                 D(i,ind((2+Hr):end,i)) = 0;
                 k_values(i, 1) = Hr;
                 ngbr_counts = ngbr_counts + Tc;
                 thisTraj = Meta(i);
                 trajNgbr(thisTraj) = trajNgbr(thisTraj) + Tc;
             else % default Isomap setting
                 D(i,ind((2+K):end,i)) = 0; 
             end
             if ((verbose == 1) & (rem(i,50) == 0)) 
                 disp([' Iteration: ' num2str(i) '     Estimated time to completion: ' num2str((N-i)*toc/60/50) ' minutes']); tic; 
...
else allow access to the following scopes: 
